...
def main():
    ...
    parser.add_argument('sa_file', type=str, help="EMIT L2B spectral abundance ENVI file")
    parser.add_argument('sa_unc_file', type=str, help="EMIT L2B spectral abundance uncertainty ENVI file")
    ...
    sa_unc_ds = envi.open(envi_header(args.sa_unc_file))
    ...
    makeGlobalAttr(nc_ds, args.sa_file, args.glt_file)

    nc_ds.summary = nc_ds.summary + \
        f"\\n\\nThis collection contains L2B estimated mineral spectral abundances \
        and geolocation data. Spectral abundances are estimated using a linear feature detector - see ATBD for  \
        details. Spectral abundance values are relative to a library reference (fractions relative to 1)."
    nc_ds.sync()

    logging.debug('Creating dimensions')
    makeDims(nc_ds, args.sa_file, args.glt_file)

    logging.debug('Creating and writing radiance metadata')
    add_variable(nc_ds, "sensor_band_parameters/minerals", str, "Mineral Names", None,
                 sa_ds.metadata['band names'], {"dimensions": ("number_of_bands",)})
    ...
    add_variable(nc_ds, 'spectral_abundance', "f4", "Spectral Abundance", "unitless", sa_ds.open_memmap(interleave='bip')[...].copy(),
                 {"dimensions":("number_of_scans", "pixels_per_scan", "number_of_bands")})
    nc_ds.sync()

    add_variable(nc_ds, 'spectral_abundance', "f4", "Spectral Abundance Uncertainty", "unitless",
                 sa_unc_ds.open_memmap(interleave='bip')[...].copy(),
                 {"dimensions":("number_of_scans", "pixels_per_scan", "number_of_bands")})
    nc_ds.sync()
