...
USGS_SpectralDataReader::USGS_SpectralDataReader(const char * fname)
{
  strncpy((char *) this->filename, (const char *)fname, MAX_STRING);
}
...
bool USGS_SpectralDataReader::cLoadRecords(){
	...
	streampos begin, end;
	ifstream lSpecFile;
	lSpecFile.open(this->filename, ios::binary);
